...
for i = 1 : length(r)
    for w = 1 : length(waves)
        ...
        [a, b] = MieScatteringCoefficients(r(i) * ...
            2 * pi / waves(w).wavelength, m(i, w));
        ...
        % Noising the pattern.
        I(i, :, w) = clearPattern; %+ normrnd(0, noise,...
            %1, length(clearPattern));
    end
    
    if toc > 10 && ~hwaitbar
        % Display waitbar if thinking more than a second
        hwaitbar = waitbar(0, 'Computing patterns matrix');
    end
    
    if hwaitbar
        waitbar(i / length(r), hwaitbar);
    end
end
